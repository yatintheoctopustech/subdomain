...
<?php
    ...
    foreach( $users as $user ){
        ...
        echo "<tr>";
            echo "<td>{$user['Member']['id']}</td>";
            echo "<td>{$user['Member']['firstname']}</td>";
            echo "<td>{$user['Member']['lastname']}</td>";
            echo "<td>{$user['Member']['username']}</td>";
            //echo "<td>{$user['User']['email']}</td>";
             
            //here are the links to edit and delete actions
            echo "<td class='actions'>";
                echo $this->Html->link( 'Edit', array('action' => 'edit', $user['Member']['id']) );
                 
                //in cakephp 2.0, we won't use get request for deleting records
                //we use post request (for security purposes)
                echo $this->Form->postLink( 'Delete', array(
                        'action' => 'delete', 
                        $user['Member']['id']), array(
